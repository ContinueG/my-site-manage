<template>
  <div class="app-container">
    ...
    <div style="margin: 15px 0">
      <el-button type="primary" @click="editArticleHandle">确认修改</el-button>
    </div>
  </div>
</template>
...
import { putOneBlog, findOneBlog, editBlog } from "@/api/blog";
import { blog } from "@/fake";
  ...
  methods: {
    ...
    editArticleHandle() {
      ...
      if (obj.title && obj.description && obj.htmlContent && obj.categoryId) {
        editBlog({ id: this.$route.params.id, data: obj }).then(() => {
          this.$message({
            showClose: true,
            message: "恭喜你，添加成功",
            type: "success",
    ...
    editOneBlog() {
      findOneBlog(this.$route.params.id).then((res) => {
        // this.from = res.data
        //this.form.select = data.category === null ? '' : data.category.id;
        // this.$refs.toastuiEditor.invoke('setHTML', data.htmlContent);
        //这里使用了假数据
        this.form = blog.data.rows[0];
        // this.form.select = blog.data.rows[0] === null ? "" : blog.data.rows[0];
      });
    },
  },
};
